...
`|C_k|`为属于类`C_k`的样本个数,`|D|`为样本容量，则数据集`D`的经验熵公式：

![](../_assets/信息增益公式1.gif)

特征`A`把`D`划分成`n`个子集`D_1,D_2,...,D_n`；记子集`D_i`中属于类`C_k`的样本集合为`D_ik`，则特征`A`对数据集`D`的经验条件熵`H(D|A)`：
...
>相比于信息增益，为什么要使用信息增益比来选择特征？

以**信息增益**作为划分训练数据集的特征，存在偏向于选择取值较多的特征的问题（如使用身份证号作为特征，则每个特征取值只有一个样本）。使用**信息增益比**可以对这一问题进行校正。

信息增益比公式：

![](../_assets/信息增益比公式1.gif)

![](../_assets/信息增益比公式2.gif)
